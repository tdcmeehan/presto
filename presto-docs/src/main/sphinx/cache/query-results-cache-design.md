...
- Partial result reuse (predicate stitching). See Future Work.
...
1. **Strip version metadata.** Connectors like Iceberg embed mutable version
   ...
   (See [prestodb/presto#26897](https://github.com/prestodb/presto/issues/26897).)
...
consulted (see Security Considerations).
...
tagging matched by a [lifecycle rule](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html)
on the `cache/` prefix).
...
See Key management options below.
...
| `IcebergTableLayoutHandle.java` | Implement `getIdentifier()` — `(catalogName, schemaName, tableName)`, stripping `snapshotId` |
...
| `SqlQueryExecution.java` | Cache check in `start()`, cache-hit serving via `serveCachedResult()` |
| `ExchangeClient.java` | Add tee-write listener for cache page capture during normal execution |
