...
public final class ArrayNgramsFunction
{
    ...
    public static Block ngrams(@SqlType("array(T)") Block array, @SqlType(StandardTypes.BIGINT) long n)
    {
        ...
        int recordsPerElement = toIntExact(min(array.getPositionCount(), n));
        int totalRecords = array.getPositionCount() - recordsPerElement + 1;
        int[] ids = new int[totalRecords * recordsPerElement];
        int[] offset = new int[totalRecords + 1];
        for (int i = 0; i < totalRecords; i++) {
            for (int j = 0; j < recordsPerElement; j++) {
                ids[i * recordsPerElement + j] = i + j;
            }
            offset[i + 1] = (i + 1) * recordsPerElement;
        }

        return ArrayBlock.fromElementBlock(totalRecords, Optional.empty(), offset, array.getPositions(ids, 0, totalRecords * recordsPerElement));
    }
}
