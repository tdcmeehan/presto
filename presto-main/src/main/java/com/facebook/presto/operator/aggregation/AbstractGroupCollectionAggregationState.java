        ...
        extends AbstractGroupedAccumulatorState
{
    ...
    /**
     * Performs the necessary recordkeeping of internal linked list to prepare for a new entry in the underlying PageBuilder.
    ...
    protected final void prepareAdd()
    {
        if (currentPageBuilder.isFull()) {
            totalPositions += currentPageBuilder.getPositionCount();
        ...
        if (totalPositions + insertedPosition == capacity) {
            capacity *= 1.5;
            nextBlockIndex.ensureCapacity(capacity);
            nextPosition.ensureCapacity(capacity);
        }
    ...
    {
        return groupEntryCount.get(getGroupId()) == 0;
    }
