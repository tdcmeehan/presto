...
public class JoinDynamicFilter
{
    ...
    /**
     * Mark this filter as not being generated by the optimizer.
     * This immediately completes the filter with TupleDomain.all(),
     * allowing consumers to proceed without waiting for timeout.
     *
     * @param reason The reason why the filter will not be generated
     */
    public void markAsNotGenerated(String reason)
    {
        synchronized (this) {
            if (!constraintByFilterIdFuture.isDone()) {
                mergedConstraint = TupleDomain.all();
