...
public class TestArrayAllocator
{
    ...
    public void testNewLease()
    {
        ...
        assertEquals(allocator.getOutstandingArrays(), 5);

        for (int i = 0; i < 5; i++) {
            allocator.returnArray(arrayList.pop());
        }
        assertEquals(allocator.getOutstandingArrays(), 0);
    }
    ...
    @Test
    public void testOverReturnLeases()
    {
        SimpleArrayAllocator allocator = new SimpleArrayAllocator(2);

        int[] array = allocator.borrowIntArray(10);
        allocator.returnArray(array);
        assertThrows(IllegalStateException.class, () -> allocator.returnArray(array));
    }
}
